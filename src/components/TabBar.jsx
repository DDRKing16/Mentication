  ...
  return (
    <nav aria-label="Primary navigation" className="pointer-events-none fixed inset-x-0 bottom-0 z-40">
      <div className="safe-x">
        <div className="pointer-events-auto mx-auto max-w-[36rem] rounded-t-[1.5rem] border-t border-[#C99646]/25 bg-[#ECE2D2]/95 px-1.5 pt-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))] shadow-[0_-14px_40px_-22px_rgba(30,60,66,0.24)] backdrop-blur-xl">
          <div className="grid grid-cols-6 items-stretch">
            {TABS.map((t) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={"h-5 w-5 " + (active ? "text-[#0E4536]" : "text-[#315E51]")}
                    strokeWidth={active ? 2 : 1.6}
                  />
                  <span className="max-w-full truncate whitespace-nowrap text-[0.65rem] font-medium leading-none" style={{ color: active ? "#0E4536" : "#315E51" }}>
                    {t.label}
                  </span>
                  {active && (
                    <span className="absolute bottom-0 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-[#7A572E]" />
                  )}
                </button>
              );
            })}
