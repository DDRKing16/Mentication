...
const RULES = Object.freeze([
  ...
  { from: "grounding54321V2", to: "reroute", when: (c) => c.orientationImproved && c.needsDestination, reason: "You are more oriented, but attention still needs somewhere safe and absorbing to go." },
  { from: "reroute", to: "nextAction", when: (c) => c.couldNotBegin || c.firstActionUnclear, reason: "The destination fits; the remaining problem is finding a startable first action." },
  { from: "reroute", to: "openChannel", when: (c) => c.destination === "social" && c.safeContact, reason: "Safe contact is the preferred destination, so Open Channel can help create a deliberate bridge." },
  { from: "countermove", to: "reroute", when: (c) => c.directActionTooDemanding, reason: "Direct action is too demanding right now; brief redirection can create enough choice for a later move." },
